...
# start simulation with Smoldyn
def smoldyn_billiard(center, radius, length, N, D, total_time, time_step, seed1, path):
    ...
    s = smoldyn.Simulation(low = [left_axis, bot_axis], high = [right_axis, top_axis], log_level = 5, quit_at_end = True)
    ...
    billiard.setStyle('both', color = 'black', thickness = 0)
    ...
    # define file name
    fname = '2D_billiard_cx_' + str(cx) + '_cy_' + str(cy) + '_len_' + str(rx-lx) + '_diam_' + str(top-bot) + '_D_' + str(D) + '_N_' + str(N) + '.npz'
    fpath = path + '\\' + fname
...
def create_map(center, radius, length, array, map_res, delta_t, time_step, D, N, path):
    ...
    # define file name
    fname = '2D_billiard_cx_' + str(cx) + '_cy_' + str(cy) + '_len_' + str(rx-lx) + '_diam_' + str(top-bot) + '_D_' + str(D) + '_N_' + str(N) + '_map_df.csv.zip'
    fpath = path + '\\' + fname
